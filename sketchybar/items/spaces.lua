...
sbar.exec("aerospace list-workspaces --all", function(spaces)
  for space_name in spaces:gmatch "[^\r\n]+" do
    ...
    space:subscribe("aerospace_workspace_change", function(env)
      local selected = tostring(env.FOCUSED_WORKSPACE) == space_name -- Ensure both are strings for comparison

      space:set {
        icon = {
          color = selected and colors.icon.highlight or colors.icon.color, -- Explicitly set colors
        },
        label = {
          color = selected and colors.label.highlight or colors.label.color,
        },
      }
