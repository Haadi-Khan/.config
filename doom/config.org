...
#+end_src
*** Typescript
Ok so basically typescript (and web dev in general) really needs multiple language servers at once (for things like tailwind completion alongside tsx) so I'm gonna use LSP mode *just* for typescript. Whenever eglot supports multiple servers at once I'll ditch it but it's the best option atm
...
;;   :config
;;   (cl-pushnew '((js-ts-mode typescript-ts-mode typescript-tsx-mode typescriptreact-mode) . ("typescript-language-server" "--stdio"))
;;               eglot-server-programs
;;               :test #'equal))
#+end_src
*** CSS
#+begin_src emacs-lisp :tangle config.el
;; (set-eglot-client! 'css-ts-mode '("vscode-css-language-server" "--stdio"))
;; (set-eglot-client! 'css-ts-mode '("tailwindcss-language-server" "--stdio"))
;; (add-hook! '(css-ts-mode-hook) #'eglot-ensure)
#+end_src
*** Swift
#+begin_src emacs-lisp :tangle config.el
(set-eglot-client! 'swift-mode '("xcrun" "sourcekit-lsp"))
